    ...
    .action(async (target: string | undefined, options: RepoCheckOptions) => {
      ...
      try {
        ...
        if (options.json) {
          ...
          printRepoHealthReport('🩺 Repository Check', report, rootOptions.color);
        }
    ...
    .action(async (target: string | undefined, options: RepoFixOptions) => {
      ...
      try {
        ...
        if (options.json) {
          ...
          printRepoHealthReport('🛠️ Repository Fix', report, rootOptions.color);

          console.log('');
          console.log(
            rootOptions.color
              ? chalk.bold.cyan('Suggested next steps')
              : 'Suggested next steps',
          );
          console.log(rootOptions.color ? chalk.gray('─'.repeat(60)) : '─'.repeat(60));
...
async function runRepoDiagnostics(
  ...
  steps.push(
    await runStep(
      'Git status',
      'git status --short',
      async () => execa('git', ['status', '--short'], { cwd }),
      true,
    ),
  );
...
function printRepoHealthReport(
  title: string,
  report: DiagnosticsReport,
  color: boolean,
): void {
  const checks: HealthCheck[] = report.steps.map((step) => ({
    name: step.label,
    status: step.skipped ? 'warn' : step.ok ? 'ok' : 'fail',
    required: !step.skipped,
    details: buildStepDetails(step),
  }));

  console.log(color ? chalk.gray(`Path: ${report.path}`) : `Path: ${report.path}`);
  console.log(color ? chalk.gray(`Timestamp: ${report.timestamp}`) : `Timestamp: ${report.timestamp}`);

  const healthReport = printHealthReport(title, checks, color);

  if (!healthReport.hasBlockingFailures) {
    return;
  }
  ...
  if (failedSteps.length) {
    console.log('');
    console.log(color ? chalk.bold.red('Failed step details') : 'Failed step details');
...
function buildStepDetails(step: StepResult): string {
  if (step.skipped) {
    return step.stdout ?? 'Skipped';
  }

  if (step.ok) {
    return step.stdout
      ? truncateMultiline(step.stdout, 140)
      : 'Completed successfully';
  }

  return truncateMultiline(
    step.stderr ?? step.stdout ?? 'Command failed',
    140,
  );
}
