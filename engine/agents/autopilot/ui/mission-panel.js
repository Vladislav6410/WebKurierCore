...
function renderActionLog() {
  ...
  if (!actionLogItems.length) {
    box.innerHTML = `<div class="action-log-empty">Лог пока пуст</div>`;
    return;
  }

  box.innerHTML = actionLogItems.map(item => `
    <div class="action-log-row">
      <div>${item.time}</div>
      <div>${item.action ?? "—"}</div>
      <div>${item.mode ?? "—"}</div>
      <div class="action-log-status ${item.result ?? "accepted"}">${item.result ?? "—"}</div>
    </div>
  `).join("");
}
...
function renderLogTail() {
  const body = document.getElementById("logTailBody");

  if (!body) return;

  if (!actionLogItems.length) {
    body.innerHTML = `<div class="action-log-empty">Записей пока нет</div>`;
    return;
  }

  const tail = actionLogItems.slice(0, 10);

  body.innerHTML = tail.map(item => `
    <div class="log-tail-row">
      <div>${item.time}</div>
      <div>${item.action ?? "—"}</div>
      <div>${item.mode ?? "—"}</div>
      <div class="log-tail-status ${item.result ?? "accepted"}">${item.result ?? "—"}</div>
    </div>
  `).join("");
}
...
async function refreshActionLog() {
  try {
    const data = await getActionLog(20);

    rawActionLogItems = Array.isArray(data.items) ? data.items : [];

    actionLogItems = rawActionLogItems.map(item => ({
      time: item.ts_iso
        ? new Date(item.ts_iso).toLocaleTimeString("ru-RU", { hour12: false })
        : "—",
      action: item.action,
      mode: item.mode,
      result: item.status
    ...
    setStatus(`log недоступен: ${error.message}`);
  }
}
