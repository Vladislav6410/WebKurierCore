...
const DEFAULTS = {
  ...
  scan: {
    enabled: true,
    subnets: [], // empty => auto-detect local /24
    from: 2,
    to: 254,
    concurrency: 60,
    timeoutMs: 700,
...
export const TelemetryBridge = {
  // Terminal API expects handler(argsLine)
  register(terminal) {
  ...
  async startLive() {
    ...
    if (WS) {
      ...
      Runtime.ws = null;
      onEvent({ type: "ws_missing", ts: nowIso(), host, hint: "npm i ws to enable websocket" });
    }

    // always do HTTP polling too (fallback + reliable)
    httpPoll(host);
