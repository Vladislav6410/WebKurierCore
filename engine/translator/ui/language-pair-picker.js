 ...
 *
 * Usage:
 *   import { mountLanguagePairPicker } from "../translator/ui/language-pair-picker.js";
 *   mountLanguagePairPicker({ mountEl, onChangePair });
 */
...
// Minimal but practical list; extend anytime (you said “all languages” later)
const LOCALES = [
  ...
  // Add more here later (we’ll expand to your 21-screenshot list)
];
...
export function mountLanguagePairPicker({
  ...
  mountEl.innerHTML = `
    <div class="wk-voice-box" style="margin-top:12px">
      ...
      <div class="wk-row">
        <label class="wk-status" style="min-width:70px">A:</label>
        ...
        <label class="wk-status" style="min-width:70px">B:</label>
  ...
  elSave.addEventListener("click", () => {
    ...
    elHint.textContent = `✅ Saved: ${saved.a} ↔ ${saved.b}`;
    emitChange({ ...pair });
  });
...
function fillSelect(selectEl, selected) {
  ...
  if (![...selectEl.options].some(op => op.value === selected)) {
...
function setHint(pair) {
  const a = pair?.a || DEFAULT_PAIR.a;
  const b = pair?.b || DEFAULT_PAIR.b;
  const autoA = a === "auto";
  const autoB = b === "auto";
  ...
  const el = document.getElementById("wk-pair-hint");
  if (el) el.textContent = `${a} ↔ ${b} • ${warn}`;
}
