  ...
  static calculate(input: RiskCalculatorInput): SecurityCheckResult {
    ...
    if (hasMalicious) {
      ...
      recommendations.push({
        code: 'AVOID_TARGET',
        message: 'Do not open or share this target',
      });
      recommendations.push({
        code: 'MANUAL_REVIEW',
        message: 'Escalate for manual security review',
      });
      ...
      recommendations.push({
        code: 'VERIFY_MANUALLY',
        message: 'Use caution and verify the target manually',
      });
      ...
      recommendations.push({
        code: 'MONITOR',
        message: 'Continue to monitor if the target is business-critical',
      });
    } else {
      riskScore += 15;
      findings.push({
      ...
      recommendations.push({
        ...
        message: 'Collect additional intelligence before trusting the target',
      });
    }

    const verdict = this.resolveVerdict(riskScore);
